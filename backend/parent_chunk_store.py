"""父级分块文档存储（用于 Auto-merging Retriever），按 kb_type 隔离"""
...
class ParentChunkStore:
    """基于 PostgreSQL + Redis 的父级分块存储，按 kb_type 隔离病历 / 说明书。"""
    ...
    def upsert_documents(self, docs: List[dict], kb_type: str) -> int:
        """写入/更新父级分块，返回写入条数。所有记录打上 kb_type 标签。"""
        if not docs:
            return 0
    ...
    def get_documents_by_ids(self, chunk_ids: List[str]) -> List[dict]:
        """按 chunk_id 批量查询。由于 chunk_id 已含 kb_type 前缀，无需额外过滤。"""
        if not chunk_ids:
            return []
    ...
    def delete_by_filename(self, filename: str, kb_type: str) -> int:
        """按文件名 + kb_type 删除父级分块，返回删除条数。"""
        if not filename:
            return 0
        ...
        try:
            rows = (
                ...
                .filter(ParentChunk.filename == filename, ParentChunk.kb_type == kb_type)
                .all()
            )
