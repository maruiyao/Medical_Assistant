...
from milvus_client import MilvusManager, VALID_KB_TYPES
...
def _validate_kb_type(kb_type: str) -> str:
    if kb_type not in VALID_KB_TYPES:
        raise HTTPException(
            status_code=400,
            detail=f"无效的 kb_type: {kb_type!r}，合法值：{sorted(VALID_KB_TYPES)}",
        )
    return kb_type


def _remove_bm25_stats_for_filename(filename: str, kb_type: str) -> None:
    ...
    get_embedding_service(kb_type).increment_remove_documents(texts)
...
def _process_upload_job(job_id: str, file_path: str, filename: str, kb_type: str) -> None:
    ...
    try:
        ...
        milvus_manager.init_collection(kb_type)
        delete_expr = f'filename == "{filename}"'
        try:
            _remove_bm25_stats_for_filename(filename, kb_type)
        except Exception:
            pass
        try:
            milvus_manager.delete(delete_expr, kb_type)
        except Exception:
            pass
        try:
            parent_chunk_store.delete_by_filename(filename, kb_type)
        except Exception:
            pass
        ...
        new_docs = loader.load_document(file_path, filename, kb_type)
        if not new_docs:
        ...
        parent_chunk_store.upsert_documents(parent_docs, kb_type)
        ...
        milvus_writer.write_documents(leaf_docs, kb_type, progress_callback=_on_vector_progress)
...
def _process_delete_job(job_id: str, filename: str, kb_type: str) -> None:
    ...
    try:
        ...
        result = milvus_manager.delete(delete_expr, kb_type)
...
async def list_documents(
    kb_type: str = Query(..., description="知识库类型：medical_record 或 medication"),
    _: User = Depends(get_current_user),
):
    """获取指定 kb_type 已上传的文档列表（登录用户）"""
...
async def upload_document_async(
    ...
    job = upload_job_manager.create_job(filename, kb_type)
    ...
    background_tasks.add_task(_process_upload_job, job["job_id"], str(file_path), filename, kb_type)
...
async def get_upload_job(job_id: str, _: User = Depends(get_current_user)):
...
async def list_upload_jobs(_: User = Depends(get_current_user)):
...
async def delete_document_async(
    ...
    background_tasks.add_task(_process_delete_job, job["job_id"], filename, kb_type)
...
async def upload_document(
    ...
    try:
        ...
        try:
            new_docs = loader.load_document(str(file_path), filename, kb_type)
        ...
        parent_chunk_store.upsert_documents(parent_docs, kb_type)
        milvus_writer.write_documents(leaf_docs, kb_type)
