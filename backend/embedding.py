...
class EmbeddingService:
    ...
    def tokenize(self, text: str) -> list[str]:
        ...
        # 改为 jieba 词级分词，避免中文按单字入 BM25 词表。
        raw_tokens = jieba.lcut(text, cut_all=False)
        tokens: list[str] = []
        for piece in raw_tokens:
            word = (piece or "").strip().lower()
            if not word:
                continue
            tokens.extend(_TOKEN_PATTERN.findall(word))
        return tokens
...
# 两个知识库各自独立的 BM25 状态，互不干扰
embedding_service_medical_record = EmbeddingService(
    state_path=_DATA_DIR / "bm25_state_medical_record.json"
)
embedding_service_medication = EmbeddingService(
    state_path=_DATA_DIR / "bm25_state_medication.json"
)

# 兼容旧代码（检索链路暂未改动）
embedding_service = embedding_service_medical_record


def get_embedding_service(kb_type: str) -> "EmbeddingService":
    """根据 kb_type 返回对应的 EmbeddingService 实例。"""
    if kb_type == "medication":
        return embedding_service_medication
    return embedding_service_medical_record
