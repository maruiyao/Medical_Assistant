...
class DocumentLoader:
    ...
    @staticmethod
    def _build_chunk_id(kb_type: str, filename: str, page_number: int, level: int, index: int) -> str:
        return f"{kb_type}::{filename}::p{page_number}::l{level}::{index}"
    ...
    def _split_page_to_three_levels(
        ...
        for level_1_doc in level_1_docs:
            ...
            if not level_1_text:
                continue
            level_1_id = self._build_chunk_id(kb_type, filename, page_number, 1, level_1_counter)
            level_1_counter += 1
            ...
            for level_2_doc in level_2_docs:
                ...
                if not level_2_text:
                    continue
                level_2_id = self._build_chunk_id(kb_type, filename, page_number, 2, level_2_counter)
                level_2_counter += 1
                ...
                for level_3_doc in level_3_docs:
                    ...
                    if not level_3_text:
                        continue
                    level_3_id = self._build_chunk_id(kb_type, filename, page_number, 3, level_3_counter)
                    level_3_counter += 1
                    root_chunks.append({
                        **base_doc,
                        "text": level_3_text,
    ...
    def load_document(self, file_path: str, filename: str, kb_type: str = "medical_record") -> list[dict]:
        """
        加载单个文档并分片
