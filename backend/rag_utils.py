...
def _retrieve_from_kb(
    query: str, kb_type: str, candidate_k: int, filter_expr: str
) -> List[dict]:
    """从单个 kb_type 执行混合检索，失败时降级为密集检索，仍失败则返回空列表。"""
...
def retrieve_documents(query: str, top_k: int = 5) -> Dict[str, Any]:
    ...
    try:
        ...
        # 去重（相同 chunk_id 保留 score 最高的）
        seen: Dict[str, dict] = {}
        for doc in all_retrieved:
            cid = doc.get("chunk_id") or id(doc)
            if cid not in seen or doc.get("score", 0) > seen[cid].get("score", 0):
                seen[cid] = doc
        retrieved = sorted(seen.values(), key=lambda d: d.get("score", 0), reverse=True)
