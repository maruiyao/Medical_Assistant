...
createApp({
    ...
    methods: {
        ...
        async handleSend() {
            ...
            try {
                ...
                if (error.name === 'AbortError') {
                    ...
                    this.messages[botMsgIdx].isThinking = false;
                    this.messages[botMsgIdx].text = `抱歉，出现了一些问题：${error.message}`;
                }
            } finally {
        ...
        currentKbType() {
            return this.activeNav === 'medicationMemory' ? 'medication' : 'medical_record';
        },

        handleMedicalRecordMemory() {
            this.activeNav = 'medicalRecordMemory';
            this.showHistorySidebar = false;
            this.loadDocuments();
        },

        handleMedicationMemory() {
            this.activeNav = 'medicationMemory';
            this.showHistorySidebar = false;
            this.loadDocuments();
        },
        ...
        async loadDocuments() {
            ...
            try {
                const response = await this.authFetch(`/documents?kb_type=${encodeURIComponent(this.currentKbType())}`);
                if (!response.ok) {
                    const data = await response.json().catch(() => ({}));
        ...
        uploadFileWithProgress(file) {
            return new Promise((resolve, reject) => {
                ...
                xhr.open('POST', `/documents/upload/async?kb_type=${encodeURIComponent(this.currentKbType())}`);
        ...
        async deleteDocument(filename) {
            ...
            try {
                const response = await this.authFetch(
                    `/documents/delete/async/${encodeURIComponent(filename)}?kb_type=${encodeURIComponent(this.currentKbType())}`,
                    { method: 'DELETE' }
                );
