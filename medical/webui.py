...
try:
    ...
    # Allow running without python-dotenv; users can still export env vars manually.
    pass

# 配置阿里 DashScope 参数
LLM_MODEL_ID = "qwen3-max" # 注意：DashScope 官方目前最强模型通常是 qwen-max
...
def load_model(cache_model):
    ...
    # 使用 map_location 参数，让 torch 自动处理设备映射
...
"""
    ...
    response = client_llm.chat.completions.create(
        model=LLM_MODEL_ID,  # 使用你定义的 qwen-max
...
def main(is_admin, usname):
    ...
    if not neo4j_uri or not neo4j_password:
        st.error("Neo4j 未配置：请在 .env 中设置 NEO4J_URI 和 NEO4J_PASSWORD（以及可选的 NEO4J_USER/NEO4J_DB）。")
        st.stop()

    client = py2neo.Graph(neo4j_uri, auth=(neo4j_user, neo4j_password), name=neo4j_db)
    ...
    if query := st.chat_input("Ask me anything!", key=f"chat_input_{active_window_index}"):
        ...
        last = ""
        # 替换原本的 ollama.chat 逻辑
        response_stream = client_llm.chat.completions.create(
            model="qwen-max",
            messages=[{'role': 'user', 'content': prompt}],
            stream=True  # 开启流式传输
        )
