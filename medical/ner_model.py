...
if __name__ == "__main__":
    ...
    # 2. 取消注释，并加入 map_location 以兼容 CPU/GPU
    # 注意：请确保文件名与你文件夹下的 .pt 文件名完全一致
    checkpoint_path = f'model/{cache_model}.pt'

    if os.path.exists(checkpoint_path):
        print(f"正在从 {checkpoint_path} 加载预训练权重...")
        # 核心修改点：加入 map_location=device
        model.load_state_dict(torch.load(checkpoint_path, map_location=device))
    else:
        print("未找到预训练模型，将使用随机初始化参数。")
